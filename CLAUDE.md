...
- 会话启动时：本文件（CLAUDE.md）由 Claude Code 自动加载；wiki 状态简报（`wiki/index.md` 摘要 + `log.md` 近 6 条 + 未 ingest 的 raw + raw_hash drift + 缺 hash）由 SessionStart hook 自动注入（见 `.claude/hooks/`）—— 无需手动读，扫一眼简报即可；若提示 drift / 未 ingest，主动按 §5 / §8 处理。
