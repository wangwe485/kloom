...
1. 本 wiki 已由 `/kloom:init-wiki` 生成（领域 type 见 CLAUDE.md §2）。
2. Obsidian 打开本目录为 vault（合规默认值已写入 `.obsidian/`：关闭 `[[Wikilinks]]`，附件目录 `raw/assets`）。
3. `cd <本路径> && claude`，然后 `/kloom:add-source raw/articles/xxx.md` 开始 ingest。
