...
---
从模板 `E:\personal\llm-wiki-template` 复制骨架到 `$1` 指定路径，创建一个主题为 "$2..." 的新 LLM Wiki。

步骤：

1. **复制**：把 `E:\personal\llm-wiki-template` 的全部内容复制到新路径 `$1`，**排除 `.git` 和 `.obsidian`**。
...
硬约束：
- **只写新路径 `$1`**，绝不修改模板源 `E:\personal\llm-wiki-template`。
